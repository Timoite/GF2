...
class Parser:
    ...
    def __init__(self, names, devices, network, monitors, scanner):
        ...
        self.error_list = [self.MISSING_ARROW_COMMA_OR_EQUALS,
                           ...
                           self.devices.BAD_DEVICE, self.MISSING_SLASH_OR_COMMA,
                           ...
                           self.network.INPUT_CONNECTED, self.network.FIRST_PORT_ABSENT,
                           self.network.SECOND_PORT_ABSENT, self.network.FIRST_DEVICE_ABSENT, 
                           self.network.SECOND_DEVICE_ABSENT, self.network.INPUT_CONNECTED,
    ...
    def _error(self, error_type, current_line, next_symbol, stopping_symbol):
        ...
        if stopping_symbol == "standard":
            while not (self.symbol.type == self.scanner.COMMA or self.symbol.type == self.scanner.KEYWORD or self.symbol.type == self.scanner.EOF):
                ...
                current_line = current_line + self.names.get_name_string(self.symbol.id)
            print("Error in line:")
            print(current_line)
            i = 0
            while i < arrow_pos:
        ...
        if error_type == self.MISSING_ARROW_COMMA_OR_EQUALS:
            print("Error: expected a right arrow, comma, keyword or equals symbol.")
    ...
    def _seek_error(self,current_line,target):
        if target == "first":
            ...
            if (current_line[equals+1:].find("-") > current_line[equals+1:].find(">")
                 and not current_line[equals+1:].find(">") == -1):
                return equals + 1
            else:
    ...
    def _portID(self, current_line):
        ...
        if self.symbol.type == self.scanner.STRING:
            ...
            if not (self.symbol.type == self.scanner.ARROW or self.symbol.type == self.scanner.COMMA or self.symbol.type == self.scanner.EQUALS or self.symbol.type == self.scanner.KEYWORD):
                self._error(self.MISSING_ARROW_COMMA_OR_EQUALS, current_line, self.names.get_name_string(self.symbol.id), "standard")
                return portID
            else:
                return portID
        else:
            self._error(self.MISSING_STRING, current_line, self.names.get_name_string(self.symbol.id), "standard")
    ...
    def _signalID(self,current_line):
        ...
        if self.symbol.type == self.scanner.DASH:
            ...
            if not [self.symbol.type == self.scanner.COMMA or self.symbol.type == self.scanner.ARROW
                     or self.symbol.type == self.scanner.DASH or self.symbol.type == self.scanner.KEYWORD]:
                self._error(self.MISSING_ARROW_DASH_OR_COMMA, current_line, self.names.get_name_string(self.symbol.id), "standard")
            else:
    ...
    def _devicetype(self, current_line):
        if self.symbol.type == self.scanner.DEVICE_TYPE:
            ...
            self._error(self.NOT_DEVICE_NAME, current_line, self.names.get_name_string(self.symbol.id), "standard")
        ...
        if self.symbol.type == self.scanner.EQUALS:
            ...
            if self.symbol.type == self.scanner.SLASH:
                ...
                if self.symbol.type == self.scanner.INTEGER:
                    ...
                    self._error(self.MISSING_INTEGER, current_line, self.names.get_name_string(self.symbol.id), "standard")
                ...
                self._error(self.MISSING_SLASH_OR_COMMA, current_line, self.names.get_name_string(self.symbol.id), "standard") 
        else:
            self._error(self.MISSING_EQUALS, current_line, self.names.get_name_string(self.symbol.id), "standard") 
    ...
    def _make_device(self, device, deviceID, qualifier):
        if device in ["AND", "OR", "NAND", "NOR", "XOR"]:
            if device == "AND":
                error = self.devices.make_device(deviceID, self.devices.AND, qualifier)
            elif device == "OR":
                error = self.devices.make_device(deviceID, self.devices.OR, qualifier)
            elif device == "NAND":
                error = self.devices.make_device(deviceID, self.devices.NAND, qualifier)
            elif device == "NOR":
                error = self.devices.make_device(deviceID, self.devices.NOR, qualifier)
            else:
                error = self.devices.make_device(deviceID, self.devices.XOR, qualifier)
        elif device == "CLOCK":
            error = self.devices.make_device(deviceID, self.devices.CLOCK, qualifier)
        elif device == "SWITCH":
            error = self.devices.make_device(deviceID, self.devices.SWITCH, qualifier)
        elif device == "DTYPE":
            error = self.devices.make_device(deviceID, self.devices.D_TYPE, qualifier)
        else:
    ...
    def _connection(self):
        ...
        if self.symbol.type == self.scanner.EQUALS:
            ...
            if self.symbol.type == self.scanner.ARROW:
                ...
                if not signal2[1] == None:
                    current_line = current_line + "-" + signal2[1]
            else:
                self._error(self.MISSING_ARROW, current_line, self.names.get_name_string(self.symbol.id), "standard")
        else:
            self._error(self.MISSING_EQUALS, current_line, self.names.get_name_string(self.symbol.id), "standard") 
        if self.error_count == 0:
            error_type = self.network.make_connection(signal1[0], self.devices.names.query(signal1[1]), signal2[0], self.devices.names.query(signal2[1]))
    ...
    def _monitor(self):
        ...
        if self.symbol.type == self.scanner.EQUALS:
            ...
            self._error(self.MISSING_EQUALS, current_line, self.names.get_name_string(self.symbol.id), "standard") 
        if self.error_count == 0:
            error_type = self.monitors.make_monitor(port[0], self.devices.names.query(port[1]))
