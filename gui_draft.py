...
class ScrollableGLPanel(wx.ScrolledWindow):
    ...
    def on_resize(self, event):
        ...
        virtual_size = [max(self.parent.GetSize()[0]-196, (self.cycles_completed+2)*20), 60]
        self.canvas.SetSize(virtual_size)
        self.SetScrollbars(1, 1, virtual_size[0], virtual_size[1])
        x, y = self.GetViewStart()
        self.canvas.SetPosition((-x, -y))
...
class Gui(wx.Frame):
    def __init__(self, title):
        ...
        self.total_cycles_text = wx.StaticText(self, wx.ID_ANY, str(self.cycles_completed))
    ...
    def AddSwitch(self, switch_id, switch_state):
        ...
        switch_radiobox.Bind(wx.EVT_RADIOBOX, lambda evt, temp=switch_id: self.OnSwitch(evt, temp))
        self.Layout()
    ...
    def AddMonitor(self, signal_name):
        ...
        self.scroll_gl = ScrollableGLPanel(self, signal_name, self.cycles_completed)
    ...
    def run_network(self, cycles):
        ...
        for canvas in MyGLCanvas.instances:
            canvas.render_signal(self.monitors.monitors_dictionary, self.devices, self.cycles_completed)
